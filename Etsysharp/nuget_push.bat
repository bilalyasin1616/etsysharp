...
dotnet pack ./Etsysharp.csproj -p:NuspecFile=./nugetfile.nuspec  -c Release --no-build
powershell -Command "(gc nugetfile.nuspec) -replace '%version%', 'package_version' | Out-File nugetfile.nuspec"
dotnet nuget push ./bin/Release/Etsysharp.%version%.nupkg --api-key %NUGET_API_KEY% --source https://api.nuget.org/v3/index.json
pause